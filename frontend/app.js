...
function selectField(label, key, val, opts, span = "") {
  const options = opts.map((o) => {
    const v = typeof o === "string" ? o : o.value, soon = typeof o === "object" && o.soon;
    return `<option value="${esc(v)}" ${v === val ? "selected" : ""}${soon ? " disabled" : ""} style="color:${soon ? "#b6bdd0" : "#14151a"}">${esc(v)}${soon ? "  ·  soon" : ""}</option>`;
  }).join("");
...
// Only XLM pairs are live (the desk delivers native XLM); others are marked "soon" + disabled.
const PAIRS = [{ value: "XLM / USDC" }, { value: "BTC / USDC", soon: true }, { value: "ETH / USDC", soon: true }];
...
function viewAudit() {
  ...
  const mcp = [["list_rfqs()", "all RFQs from on-chain state (USDC bands)", "RFQ[]"], ["read_settlement(rfq)", "settled outcome the on-chain ZK verifier accepted", "{settled, winner, clearingUsdc}"], ["clearing_price(rfq)", "public clearing price (USDC)", "{clearingUsdc}"], ["bid_count(rfq)", "sealed bid count", "number"], ["mark_price(sym)", "live Reflector USD mark", "{usd, timestamp}"]];
  ...
  if (m.type === "bid") {
    ...
    return wrap(`<div style="width:440px;max-width:100%;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 40px 90px -30px rgba(20,21,40,.6);animation:segelPop .18s ease-out">
      ...
      <div style="padding:20px 22px">
        <div style="display:flex;justify-content:space-between;font-size:11px;color:#8a8f9c;margin-bottom:8px"><span>your sealed bid (${r.baseLot ? "total USDC for the lot" : "USDC"})</span><span>band ${usd(r.bandMin)}–${usd(r.bandMax)}</span></div>
